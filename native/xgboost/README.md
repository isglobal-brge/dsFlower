...
v2 and binds the complete public training shape.  The third adds a deterministic
fixed-point histogram core behind a test-only compile switch and moves the ABI
to v3.  A normal build remains fail closed:
...
- even a complete context cannot train unless the isolated test-only switch is
...
The test-only core counts successful trees across Boosters sharing one installed
...
The test switch is deliberately not connected to dsFlower's public capabilities
or runner.  `grow_dsflower_dp_hist` must not be advertised or used in production
...
The complete test also builds `native/dp_primitives` with Rust 1.88 and links a
separate `DSFLOWER_DP_CORE_TESTING=ON` XGBoost library; the production build
above never enables that switch.
