"""Flower ClientApp for Federated Poisson Regression.

Count data modeling (hospital events, readmissions, adverse events).
Uses Poisson negative log-likelihood loss with a log link function.
"""
...
def _build_poisson_model(input_dim, hidden_layers=None):
    """Build a Poisson regression model (linear or MLP with exp output)."""
    ...
    def fit(self, parameters, config):
        ...
        for _ in range(self.local_epochs):
            for X_batch, y_batch in self.trainloader:
                ...
                log_rate = self.model(X_batch).squeeze(1)
                loss = self.criterion(log_rate, y_batch)
                loss.backward()
                self.optimizer.step()
    ...
    def evaluate(self, parameters, config):
        ...
        with torch.no_grad():
            for X_batch, y_batch in self.trainloader:
                ...
                y_batch = y_batch.to(self.device)
                log_rate = self.model(X_batch).squeeze(1)
                batch_loss = self.criterion(log_rate, y_batch)
...
def client_fn(context: Context) -> FlowerClient:
    ...
    input_dim = X.shape[1]
    model = _build_poisson_model(input_dim, hidden_layers)
